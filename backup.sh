...
if ! echo "$COMMITMSG" | grep "%d" > /dev/null; then # if commitmsg didn't contain %d, grep returns non-zero
...
# Commented out because not currently working on K1's version of busybox sh.
#diff_lines() {
#  local path=
#  local line=
#  local previous_path=
#  while read -r; do
#    esc=$'\033'
#    if echo "$REPLY" | grep -E ---\ (a/)?"([^[:blank:]$esc]+)".* > /dev/null; then
#      previous_path=$(echo "$REPLY" | sed -E "s/.*--- (a\/)?([^[:blank:]$esc]+).*/\2/")
#      continue
#    elif echo "$REPLY" | grep -E \+\+\+\ (b/)?"([^[:blank:]$esc]+)".* > /dev/null; then
#      path=$(echo "$REPLY" | sed -E "s/.*\+\+\+\ (b\/)?"([^[:blank:]$esc]+)".*/\2/")
#    elif echo "$REPLY" | grep -E @@\ -[0-9]+(,[0-9]+)?\ \+([0-9]+)(,[0-9]+)?\ @@.* > /dev/null; then
#      line=$(echo "$REPLY" | sed -E "s/.*@@\ -[0-9]+(,[0-9]+)?\ \+([0-9]+)(,[0-9]+)?\ @@.*/\2/")
#    elif echo "$REPLY" | grep -E ^"($esc\[[0-9;]+m)"*([\ +-]) > /dev/null; then
#      REPLY=$(echo "$REPLY" | sed -E "s/^($esc\[[0-9;]+m)*([\ +-])/\2/") # limit the line width, so it fits in a single line in most git log outputs
...
eval "$INW $INW_ARGS" | while read -r line; do
  ...
  (
    ...
    if [ -n "$DATE_FMT" ]; then
      ...
      FORMATTED_COMMITMSG=$(echo "$COMMITMSG" | awk -v date="$(date "$DATE_FMT")" '{gsub(/%d/, date)}1') # splice the formatted date-time into the commit message
    fi
